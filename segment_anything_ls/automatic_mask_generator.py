...
class SamAutomaticMaskGenerator:
    ...
    def _generate_masks(self, image: np.ndarray) -> MaskData:
        ...
        data_m, data_l = MaskData(), MaskData()
        for crop_box, layer_idx in zip(crop_boxes, layer_idxs):
            crop_data_m, crop_data_l = self._process_crop(image, crop_box, layer_idx, orig_size)
    ...
    def _process_crop(
        ...
        for (points,) in batch_iterator(self.points_per_batch, points_for_image):
            batch_data_m, batch_data_l = self._process_batch(points, cropped_im_size, crop_box, orig_size)
            data_m.cat(batch_data_m)
            data_l.cat(batch_data_l)
            del batch_data_m, batch_data_l
